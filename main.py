...
new_data_tensor = torch.tensor([[item['Attention'], item['Meditation'], item['Signal'], item['Delta'], item['Theta'],
...
def update_model():
    if request.method == 'POST':
        ...
        # Предполагаем, что данные поступают в формате списка моделей
        for model_data in models_data:
            # Извлекаем данные для обучения модели из списка
            X = []
            y = []
            print(model_data)
            X.append([model_data['Attention'], model_data['Meditation'], model_data['Signal'], model_data['Delta'], model_data['Theta'],
                      model_data['LowAlpha'], model_data['HighAlpha'], model_data['LowBeta'], model_data['HighBeta'], model_data['LowGamma'],
                      model_data['HighGamma']])
            y.append(model_data['EventName'])
        ...
        new_model = EegClassifier(input_size=len(X[0]), num_classes=len(set(y)))
        criterion = nn.CrossEntropyLoss()
        optimizer = optim.Adam(new_model.parameters(), lr=0.001)

        # Создание пользовательского Dataset
        new_dataset = EegDataset(X_tensor, y_tensor)
